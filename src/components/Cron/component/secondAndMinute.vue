...
<template lang="html">
  <div :val="value_">
    <div>
      <Radio v-model="type" label="1" size="small" border>每{{ lable }}</Radio>
    </div>
    <div>
      <Radio v-model="type" label="2" size="small" border>周期</Radio>
      <span style="margin-left: 10px; margin-right: 5px;">从</span>
      <InputNumber v-model="cycle.start" :min="1" :max="59" size="small" style="width: 100px;"@on-change="type = '2'" />
      <span style="margin-left: 5px; margin-right: 5px;">至</span>
      <InputNumber v-model="cycle.end" :min="2" :max="59" size="small" style="width: 100px;"@on-change="type = '2'" />
      {{ lable }}
    </div>
    <div>
      <Radio v-model="type" label="3" size="small" border>循环</Radio>
      <span style="margin-left: 10px; margin-right: 5px;">从</span>
      <InputNumber v-model="loop.start" :min="0" :max="59" size="small" style="width: 100px;"@on-change="type = '3'" />
      <span style="margin-left: 5px; margin-right: 5px;">{{ lable }}开始，每</span>
      <InputNumber v-model="loop.end" :min="1" :max="59" size="small" style="width: 100px;"@on-change="type = '3'" />
      {{ lable }}执行一次
    </div>
    <div>
      ...
      <Checkbox-group v-model="appoint">
        <div v-for="i in 6" :key="i" style="margin-left: 10px; line-height: 25px;">
          <Checkbox v-for="j in 10" :key="j" :label="(i - 1) + '' + (j - 1)"@on-change="type = '4'" />
        </div>
      </Checkbox-group>
    </div>
  </div>
