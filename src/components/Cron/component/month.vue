<template lang="html">
  <div :val="value_">
    <div>
      <Radio v-model="type" label="1" size="small" border>每月</Radio>
    </div>
    <div>
    ...
    <div>
      ...
      <InputNumber v-model="cycle.start" :min="1" :max="12" size="small" style="width: 100px;"@on-change="type = '2'" />
      <span style="margin-left: 5px; margin-right: 5px;">至</span>
      <InputNumber v-model="cycle.end" :min="2" :max="12" size="small" style="width: 100px;"@on-change="type = '2'" />
      月
    </div>
    <div>
      <Radio v-model="type" label="3" size="small" border>循环</Radio>
      <span style="margin-left: 10px; margin-right: 5px;">从</span>
      <InputNumber v-model="loop.start" :min="1" :max="12" size="small" style="width: 100px;"@on-change="type = '3'" />
      <span style="margin-left: 5px; margin-right: 5px;">月开始，每</span>
      <InputNumber v-model="loop.end" :min="1" :max="12" size="small" style="width: 100px;"@on-change="type = '3'" />
      月执行一次
    </div>
    <div>
      <Radio v-model="type" label="4" size="small" border>指定</Radio>
      <Checkbox-group v-model="appoint" style="margin-left: 0px;  line-height: 25px;">
        <Checkbox v-for="i in 12" :key="i" :label="i"@on-change="type = '4'" />
      </Checkbox-group>
    </div>
  </div>
</template>
