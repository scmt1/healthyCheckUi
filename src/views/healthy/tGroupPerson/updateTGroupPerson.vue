            ...
            <Tabs class="tabs">
                <TabPane label="基本信息" v-if="physicalType == '健康体检' || physicalType == '从业体检'">
                    ...
                    <Row>
                        <Col span="12" v-if="tGroupPersonForm.id">
                            <FormItem label="单位名称" prop="groupUnitId">
                                ...
                                <Select v-model="tGroupPersonForm.orderId" :filterable="true" :disabled="disabled"
                                    ...
                                    <Option v-for="(item,index) in orderArr" :key="index" :value="item.id" :tag="item.groupUnitId"
                                        ...
                                        {{ item.groupUnitName + item.createTime }}
                                    </Option>
                                </Select>
                            </FormItem>
                        </Col>
                ...
                <TabPane label="基本信息" v-if="physicalType == '职业体检' || physicalType == '放射体检'">
                    ...
                    <Row>
                        <Col span="24">
                            <FormItem label="分组" prop="groupId">
                                <Select :disabled="disabled" @on-change="groupChange" transfer v-model="tGroupPersonForm.groupId" placeholder="请选择分组">
                                    <Option :value="item.id" v-for="(item,index) in groupList" :key="index">{{item.name}}</Option>
                                </Select>
                            </FormItem>
                        </Col>
...
import {getTOrderGroupByGroupOrderId,getTOrderGroupByGroupUnitId} from "../../../api/healthy/tOrderGroup"
...
export default {
        ...
        groupChange(v){
            if(this.tGroupPersonForm.isPass && this.tGroupPersonForm.isPass != 1){
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        this.$Modal.remove();
                        this.tGroupPersonForm.oldGroupId = v;
                        this.loading = false;
                    },
                    onCancel: () => {
            ...
            this.$refs['tGroupPersonForm'].validate((valid) => {
                if (valid) {
                    ...
                    if (this.physicalType == '职业体检' || this.physicalType == '放射体检') {
                        this.tGroupPersonForm.hazardFactors = this.tGroupPersonForm.hazardFactors.join("|");
                        this.tGroupPersonForm.hazardFactorsText = this.tGroupPersonForm.hazardFactorsText.trim();
                        this.tGroupPersonForm.workStateText = this.tGroupPersonForm.workStateText.trim();
                    } else {
                        this.tGroupPersonForm.hazardFactors = null;
                    }

                    if (this.tGroupPersonId != null && this.tGroupPersonId.trim().length > 0) {
                        this.tGroupPersonForm.id = this.tGroupPersonId;
                        this.tGroupPersonForm.oldGroupId = this.tGroupPersonForm.groupId;
    ...
    watch: {
        ...
        async show(val) {
            ...
            if (val) {
                this.getGroup(this.tGroupPersonForm.orderId);
                if (this.tGroupPersonForm.groupId && !this.tGroupPersonId) {
                ...
                if (this.tGroupPersonId != null && this.tGroupPersonId.trim().length > 0) {
                    groupPersonApi.getTGroupPerson({id: this.tGroupPersonId}).then(res => {
                        if (res.code == 200) {
                            this.tGroupPersonForm = res.data;
                            if(this.tGroupPersonForm && this.tGroupPersonForm.hazardFactors){
                                this.tGroupPersonForm.hazardFactors = this.tGroupPersonForm.hazardFactors.split("|");
                            }
                            if(this.groupList.length == 0){
                                this.getGroupByUnitId(this.tGroupPersonForm.unitId);
                            }
                            ...
                            if (res.success) {
                                ...
                                for (let i = 0; i < this.data.length; i++) {
                                    if (this.data[i].title == "其他") {
                                        ...
                                        this.tGroupPersonForm.workName = "操作工";
                                        break;
                                    }
                                }
                            }
