        ...
        <div class="wrap">
            <Row :gutter="4">
                <Col v-if="expand" :style="leftSpan">
                    ...
                    <Card class="card">
                        <div style="display: flex;flex-wrap: nowrap;align-items: center;margin-bottom:10px;">
                        ...
                        <Input suffix="ios-search" placeholder="请输入关键字" clearable @input="groupOrderInputChange"/>
                        ...
                        <Card class="maxHeight" style="height: 100%;">
                            <el-tabs type="border-card" @tab-click="tabClick" v-if="groupData.length>0" v-model="groupPersonTabValue">
                                <el-tab-pane v-for="(item,index) in groupData" :key="index" :name="item.id"
                                             :label="item.name+'('+(item.personCount?item.personCount:0)+')人'">
...
export default {
    ...
    methods: {
        ...
        getGroupOrderData() {
			...
			//输入了就不带时间
			if(this.groupOrderSearchForm.groupUnitName && this.groupOrderSearchForm.groupUnitName.trim().length>0 ){
				this.groupOrderSearchForm.startDate = null;
				this.groupOrderSearchForm.endDate = null;
                this.date = '';//取消时间筛选
			}
            //this.groupOrderSearchForm.auditState = 88;
            queryTGroupOrderList(this.groupOrderSearchForm).then(res => {
                if (res.success) {
