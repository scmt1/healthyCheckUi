        ...
        methods: {
            ...
            getReport(isPreviewData,templateData) {
                ...
                if (templateData && templateData.name) {
					...
					_templateData.termOfValidity = formartDate(templateData.termOfValidity, 'yyyy-MM-dd');//有效期
					_templateData.code = templateData.code;//编号
					_templateData.unitOfIssue = templateData.unitOfIssue;//发证单位
					_templateData.headImg = templateData.headImg;//头像图片 base64
                }
                testNumGeneratorBarcode({personId:templateData.personId,testNum:templateData.code}).then(res =>{
                    if(res.success){
						...
						_templateData.barCodeImg = res.data;//条形码图片 base64
                    }
