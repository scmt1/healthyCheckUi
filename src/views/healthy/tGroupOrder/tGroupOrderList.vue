    ...
    <Row class="groupOrder">
        <Col v-show="expand" :style="leftSpan">
            <Card style="height: calc(100vh - 80px);overflow: hidden;">
                ...
                <Row>
                    <RadioGroup v-model="searchForm.payStatus" @on-change="search" style="margin-top: 10px;margin-left: 5px;">
        ...
        <Col :style="span">
            <Card style="height: calc(100vh - 80px);overflow: auto;">
                <div style="margin-bottom: 10px;">
                    ...
                    <Button type="info" @click="uploadShow = true" v-if="groupOrderForm.isAuditState == 1 && groupOrderForm.id" icon="ios-cloud-upload-outline">附件上传
                    </Button>
                    ...
                    <Button type="info" icon="ios-cloud-upload-outline" :loading="saveLoading"
                            ...
                            v-if=" groupOrderForm.id && groupOrderForm.isAuditState == 1">
                        委托协议预览
                    </Button>
                    ...
                    <Row>
                        ...
                        <Col :span="6">
                            <FormItem label="体检类型" prop="physicalType">
                                ...
                                <!--                <Select v-model="groupOrderForm.physicalType">-->
                                <!--                  <Option v-for="item in physicalTypeList" :value="item.value" :key="item.value">-->
                                <!--                    {{ item.title }}-->
                                <!--                  </Option>-->
                                <!--                </Select>-->
                            </FormItem>
                        ...
                        <Col :span="6">
                            <FormItem label="零星体检" prop="sporadicPhysical">
                                <Select v-model="groupOrderForm.sporadicPhysical" label-in-value
                        ...
                        <Col :span="6" >
                            <FormItem label="是否合同评审" prop="isAuditState" :label-width="124">
                                <!--<Select v-model="groupOrderForm.isAuditState" label-in-value>
                                    <Option v-for="item in typeList" :value="item.id" :key="item.id">{{ item.type }}
                                    </Option>
                                </Select>-->
                                <RadioGroup v-model="groupOrderForm.isAuditState">
                                        ...
                                        <Row style="margin-top: 16px;">
                                            <Col :xs="{span:24}" :md="{span:11,offset:1}" :lg="{span:7,offset:1}" :xl="{span:5,offset:1}">
                                                <FormItem label="必检项折扣：" v-if="isShow">
                                                    <InputNumber :min="1" :max="100" v-model="item.discount" v-width="100"
...
<script>
    ...
    export default {
        ...
        components: {ChoosePortfolioProject, ChooseCombo, GroupEdit, ChooseUser, previewTemplate, noData,fileUpload,orderFlow,excelUpload,contractPreview},
        computed: {
            physicalType() {
        ...
        methods: {
            ...
            checkCombo() {
                ...
                if (comboIds && comboIds.length > 0) {
                    ...
                    for (let i = 0; i < ids.length; i++) {
                        this.comboIdList.push({id: ids[i]})
                    }
                }
            ...
            sumTotal() {
                ...
                this.subtotal = this.accMul(subtotal, Number(this.groupData[this.tabValue].personCount));
            },
