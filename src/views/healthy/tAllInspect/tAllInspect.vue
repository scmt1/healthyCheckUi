            ...
            <Col :style="outLeftSpan" v-if="outExpand">
                <Card class="card">
                    ...
                    <RadioGroup type="button" size="small" class="btngroup" button-style="solid" v-model="date"
                        ...
                        <Radio label="重置" @click="resetSearchForm" style="cursor: pointer;"></Radio>
                    </RadioGroup>
            ...
            <Col :style="outSpan" height="calc(100vh - 128px)" v-if="personInfo.id">
                ...
                <Card class="inspect">
                    ...
                    <Row :gutter="50" style="margin-top: 10px">
                        <Col span="12"
                            ...
                            <Button class="choice" type="primary"
                                    @click="handleWordChoose('zjzybmc')">选
                            </Button>
                            <div class="txt"><span style="color: red;font-size: 16px;position: relative;top: 4px;">*</span>{{nametjjl}}</div>
                    ...
                    <Row :gutter="50" v-if="(personInfo.physical_type == '职业体检' || personInfo.physicalType == '职业体检') || (personInfo.physical_type == '放射体检' ||
                        ...
                        <Col span="12">
                            <div class="choice" type="primary" style="bottom: 2px;right: 45px;width: 150px;display: flex;flex-direction: row;justify-content: right;">
                                <div v-if="isContraindication">
                                    <Button type="primary" style="bottom: 2px;width: 80px;height: 20px;display: flex;flex-direction: row;justify-content: center;"
                                            @click="handleWordChooseOccupation('zyjjz')" v-if="isContraindication">职业禁忌症
                                    </Button>
                                </div>
                                <div v-if="isDiseases">
                                    <Button type="primary" style="bottom: 2px;width: 50px;height: 20px;display: flex;flex-direction: row;justify-content: center;"
                                            @click="handleWordChooseDiseases('zyb')" v-if="isDiseases">职业病
                                    </Button>
                                </div>
                            </div>
                            <Button class="choice" type="primary" style="bottom: 2px;left: 25px;width: 65px;margin-right: 0px;"
                                            ...
                                            <Col style="display: flex;margin: auto 0;line-height: 32px;">
                                                <div style="display: flex;">
                                                    ...
                                                    <img style="width: 100px;height: 30px;"
                                                         :src="inspectForm.inspectionAutograph ? 'data:image/png;base64,' + inspectForm.inspectionAutograph:doctorInfo.autograph">
                                                </div>
<!--                                                <div style="margin-left: 20px;white-space: nowrap;">总检日期：{{ inspectForm.inspectionDate }}</div>-->
                                            </Col>
                                            <Col>
                                                <FormItem label="总检日期：">
                                                    <DatePicker type="datetime" v-model="inspectForm.inspectionDate" transfer placeholder="请选择总检日期" style="width: 150px"></DatePicker>
                                                </FormItem>
                                            </Col>
                                        ...
                                        <div class="items-result">
                                            <Row>
                                                <Col span="6">
                                                    <div style="display: flex;">
                                                        <div>检查医生：</div>
                                                        <img :src="'data:image/png;base64,' + item.checkSign" width="150" height="30">
                                                    </div>
                                                </Col>
                                                <Col span="4">
        ...
        <Modal title="复查结论" v-model="reviewPageShow" :mask-closable="false" footer-hide :width="650">
            <Row :gutter="20">
                <Col span="24">
                    <Input :rows="4" type="textarea" v-model="reviewData"
                           placeholder="请输入复查结论"/>
                </Col>
            </Row>
            <Row>
        ...
        <auditing-report v-model="auditingReportShow" :person-id="personInfo.id"></auditing-report>
        ...
        methods: {
            ...
            getSelectedWordsDiseases(word, wordCode, cType) {
                ...
                if(this.inspectForm.conclusion.indexOf(";\n") > -1){
                    ...
                    if(conclusionSplit && conclusionSplit.length > 0){
                        for(let i = 0;i < conclusionSplit.length;i ++){
                            if(conclusionSplit[i].indexOf("职业病") > -1){
                                conclusionSplit[i] = "职业病：" + this.occupationalDiseasesNow;
                            }
                        }
                    }
                    this.inspectForm.conclusion = conclusionSplit.join(';\n');
                    ...
                    this.inspectForm.conclusion = "职业病：" + this.occupationalDiseasesNow;
                    this.TTemplateId.conclusion = this.inspectForm.conclusion;
                }
            },
            outChangeExpand() {
            ...
            radioChooseClick(item) {
                ...
                //this.inspectForm.handleOpinion = "无"
                // let checkResultNow = item.checkResult;//体检结果（0-未见异常，1-其他异常，2-职业禁忌症，3-疑似职业病）
            ...
            //获取分组
            getResultByGroupId(officeId, startDate, endDate) {
                this.groupShow = true;
            ...
            async getPersonList() {
                ...
                if(this.isReviewer){
                    getInspectionTGroupPersonReviewList(this.personParam).then(res => {
                        if (res.success) {
                            ...
                            if (this.personList.length > 0) {
                                ...
                                this.getCrisisProject();
                                this.getAbandonCheckProject();
                                // let checkResultNow = this.personList[0].checkResult;//体检结果（0-未见异常，1-其他异常，2-职业禁忌症，3-疑似职业病）
                    ...
                    getInspectionTGroupPersonList(this.personParam).then(res => {
                        if (res.success) {
                            this.personList = res.data.records;
                            this.personTotal = res.data.total;
                            this.inspectForm.id = "";
                        ...
                        // if (_this.personInfo.physical_type == "从业体检") {
                            ...
                            if(_this.status != '已总检' && !_this.isReviewResult && _this.personInfo.is_recheck != 1){
                                if (_this.personInfo.physical_type == "职业体检" || _this.personInfo.physical_type == "放射体检") {
                                    ...
                                    if(isDefaultValue){
                                        _this.inspectForm.conclusion = "本次职业检查所检项目未见异常；";
                    ...
                    if (res.success) {
                        ...
                        for (let i = 0; i < this.tableData.length; i++) {
                            ...
                            if (t.diagnoseSum && t.diagnoseSum != "") {
                                ...
                                if(t.diagnoseSum.indexOf("未见明显异常") == -1){
									if(!this.checkPhysical(t.diagnoseSum)){
										if (t.diagnoseSum.lastIndexOf(";") > -1 || t.diagnoseSum.lastIndexOf("；") > -1) {
											str += t.groupItemName + ":" + t.diagnoseSum + "\n";
										} else {
											str += t.groupItemName + ":" + t.diagnoseSum + ";\n";
										}
                                    }
