                                        ...
                                        <Row :gutter="6">
                                            <Col :span="12">
                                                ...
                                                <div class="border-green"
                                                     v-if="item.physicalType.indexOf('健康') != -1">
                                                    健
                                                </div>
                                                <div class="border-yellow"
                                                     v-if="item.physicalType.indexOf('从业') != -1">
                                                    从
                                                </div>
                                ...
                                <div v-if="item.id == tabsValue">
                                    ...
                                    <Form :label-width="90" style="margin-top: 30px;">
                                        <Row style="flex-wrap: nowrap;">
                                            <Col span="5">
                                                <FormItem label="检查医生">
                                                    <img :src=" item.departResult.checkSign ? 'data:image/png;base64,' + item.departResult.checkSign :
                                                checkDoctor" style="width: 150px;height: 30px;">
                                                </FormItem>
                                            </Col>
                                            <Col span="5" v-if="item.departResult.checkDate">
                                                <FormItem label="检查日期">
                                                    {{ item.departResult.checkDate.substr(0, 10) }}
                                                </FormItem>
                                            </Col>
                                            <Col span="5" v-else>
                                                <FormItem label="检查日期">
                                                    {{ checkDate.substr(0, 10) }}
                                                </FormItem>
                                            </Col>
                                            <Col span="4"></Col>
                                            <Col>
...
<!--                                                    </Button>-->
                    ...
                    <!--术语选择-->
                    <drawer v-model="wordChooseShow" :modalTitle="title" @getSelectedWords="getSelectedWords"
...
<script>
    ...
    import {addRelationPersonProjectCheck, updateRelationPersonProjectCheck} from "@/api/healthy/relationPersonProjectCheck"
        ...
        methods: {
            ...
            handleCrisisData(item) {
                ...
                let temp = this.data.filter(i => {
                    return (i.departItemResults) && (i.departItemResults.crisisDegree == '高于正常值' || i.departItemResults.crisisDegree == '低于正常值' || i.departItemResults.crisisDegree == '异常');
                })
                ...
                if (temp.length > 0) {
                    ...
                    temp.forEach(i => {
                        ...
                        if(i.departItemResults.crisisDegree && (i.departItemResults.crisisDegree.indexOf("高于")>-1||i.departItemResults.crisisDegree.indexOf("低于")>-1)){
							...
							//电测听只监听平均
								...
								if(i.name.indexOf("平均")>-1 && i.name.indexOf("骨导") <= -1){
                    ...
                    if(tab.name && (tab.name .indexOf("电测")>-1||tab.name .indexOf("纯音")>-1)){
						this.$set(tab.departResult, "diagnoseSum", "听力正常");
                    }
                    else {
			 ...
			 */
			abandonRensonClick(){
				if(this.abandonRenson && this.abandonRenson.trim().length==0){
            ...
            getPersonByOfficeId() {
                ...
                if(this.isReviewer && !this.checkFlag){
					getPersonReviewerByOfficeId(this.groupPersonSearchForm).then(res => {
						if (res.success) {
					...
					getPersonByOfficeId(this.groupPersonSearchForm).then(res => {
						if (res.success) {
							this.personData = res.data.records;
							this.personTotal = res.data.total;
							if (this.personData && this.personData.length > 0) {
            ...
            getOnePersonData(id) {
                ...
                if (this.checkFlag) {
                    ...
                    } else if (this.personPhysicalType == "健康体检") {
                        getTGroupPersonById({id: id, type: "healthy"}).then(res => {
                            if (res.success) {
                                if (res.data) {
                                    //人信息
                                    this.personInfo = res.data;
            ...
            getItemDataByGroupId() {
                ...
                getItemByGroupId({groupId: groupId, personId: personId}).then(res => {
                    if (res.success) {
                        //重新赋值，满足检查结果表字段
                        if (res.data.length > 0) {
                            this.tabsArr = [];
                            this.groupItemIdList = [];

                            //组合项目
                            res.data.forEach((i, indx) => {
                                this.groupItemIdList.push(i.id);
                                //初始化选中
                                if (indx === 0) {
                                    this.tabsValue = i.id;
                                }
                            ...
                            if (res.data && res.data.length > 0) {
                                ...
                                res.data.forEach(i => {
                                    if (i.departItemResults == null) {
                                        i.departItemResults = {};
                                        if(i.name && (i.name.indexOf("听力(左)") > -1 || i.name.indexOf("听力(右)") > -1)){
                    ...
                    if (tab.isFile == '是') {
                        if(i.departItemResults){
                            ...
                            if(i.departItemResults.result && i.departItemResults.result.trim().length>0 && physicalSummary){
                                if(physicalSummary.indexOf(i.departItemResults.result.trim())>-1){
            ...
            dataFgnContent() {
                if (this.data) {
                    ...
                    let shortName2 = this.$hospitalName.fgnTemplate.shortName.shortName2;//用力呼气肺活量占预计值百分比 简称
                    let shortName3 = this.$hospitalName.fgnTemplate.shortName.shortName3;//一秒用力呼气容量 简称
                    let shortName4 = this.$hospitalName.fgnTemplate.shortName.shortName4;//一秒率(第一秒钟用力呼气容积占预计值百分比) 简称
                    let shortName5 = this.$hospitalName.fgnTemplate.shortName.shortName5;//第一秒用力呼气率 简称
                    let dataFEVone = "";
                    let dataFVC = "";
                    /*用力呼气肺活量占预计值百分比 计算*/
                    let dataFvcBfb = "";
                    ...
                    this.data.forEach(i => {
                        ...
                        if (i && i.shortName == shortName2) {
                            if(i.departItemResults && i.departItemResults.result && i.departItemResults.result != 0 && this.$hospitalName.isUpdateUnitPage){
                                //不计算
                            }else{
                        ...
                        if (i && i.shortName == shortName5) {
                            if(i.departItemResults && i.departItemResults.result && i.departItemResults.result != 0 && this.$hospitalName.isUpdateUnitPage){
                                ...
                                if(dataFEVone != 0 && dataFVC != 0){
                                    i.departItemResults.result = ((dataFEVone/dataFVC).toFixed(2)*100).toFixed(0);
                                }else {
            ...
            calAge(scope) {
                ...
                if(tc && this.personInfo && this.personInfo.sex){
                    ...
                    if(hazardFactorsText && hazardFactorsText.indexOf("|") > -1){
                        //当前拆分后的危害因素 集合
                        let hazardFactorsTextNow = hazardFactorsText.split("|");
                        //不修正危害因素 数目
                                ...
                                if ('是' == tab.isFile) {
                                    ...
                                    this.data.forEach(item => {
                                        for (let i = 0; i < parse.length; i++) {
                                            if (tab.portfolioProjectId == "1361" && parse[i].site) {
                                                    ...
                                                    parse[i].value.forEach(j => {
                                                        if(i > 2){
                                                            if (('gl' + j.frequency) == item.relationCode) {
                                                                ...
                                                                // item.departItemResults.result = Number(j.measuredValue);
                                    ...
                                    if (tab.portfolioProjectId == "1479") {
                                        this.dataFgnContent();
                                    }
                                }
                            }
                        ...
                        if (res.success) {
                            if (res.data) {
                                if (res.data.pacsData == null) {
                                    _this.$Message.error("未查询到体检结果，无法同步数据！");
                                }
                                ...
                                if (tab.isFile == '是') {
                                    ...
                                    let conclusion = res.data.conclusion;
                                    if (conclusion != null) {
                                        let find = _this.tabsArr.find(item => item.id == _this.tabsValue);
                                        if (find) {
                                            find.departResult.diagnoseSum = conclusion.comments;
                                            find.departResult.diagnoseTip = conclusion.seeing;
                                        }
                                    } else {
                                        _this.$Message.error("未查询到体检结论！");
                                    }
                                }else{
                                    let conclusion = res.data.conclusion;
                                    if (conclusion != null) {
                                        let find = _this.tabsArr.find(item => _this.id == this.tabsValue);
                                        if (find) {
                                            find.departResult.diagnoseSum = conclusion.comments;
                                            find.departResult.diagnoseTip = conclusion.seeing;
                                        }
                                ...
                                if (parseLis.length > 0) {
                                    parseLis.forEach(item => {
                                        if(_this.lisData.length <= 4){
                                            _this.lisData.push(item.base64);
                                        }
                                        // this.lisData.push(item.base64);
                                    });
                                    if (_this.lisData && _this.lisData.length > 0) {
                                        _this.$Message.success("同步成功！");
                                    }
                                }
                                /*if (res.data.pacsData.item == "血常规") {
                                    let parseLis = parse.filter(item => item.type == 'image');
                                    parseLis.forEach(item => {
                                ...
                                if (res.data.pacsData.item.indexOf("眼科") > -1) {
                                    let parseLis = parse.filter(item => item.type == 'image');
                                    parseLis.forEach(item => {
                                        if (this.lisData.length <= 4) {
                                            this.lisData.push(item.base64);
                                ...
                                if ('是' == tab.isFile) {
                                    ...
                                    if (_this.pacsData && _this.pacsData.length > 0) {
                                        _this.$Message.success("同步成功！");
                                    }
                                } else {
                                    _this.data.forEach(item => {
                                        for (let i = 0; i < parse.length; i++) {
                                            if (parse[i].shortName == item.relationCode) {
                                                if ("neg" != parse[i].result) {
                                                    ...
                                                    if(parse[i].row){
                                                        if(parse[i].scope){
                                                            _this.$set(item.departItemResults, 'scope', parse[i].scope);
                                                        }
                                                        switch (parse[i].row){
                                                          case "L":{
                                                        ...
                                                        _this.$set(item.departItemResults, 'crisisDegree', '-');
                                                        _this.$set(item.departItemResults, 'arrow', '-');
                                                    } else {
                                                        let rett = _this.setResult(item.criticals, parse[i].result);
                                                        if (rett.flag == "-") {
                                                            _this.$set(item.departItemResults, 'crisisDegree', '-');
                                                            _this.$set(item.departItemResults, 'arrow', '-');
                                                        } else if (rett.flag) {
                                                            _this.$set(item.departItemResults, 'crisisDegree', rett.arrow == '-' ? rett.resp?rett.resp: '-'  : rett.arrow == '↑' ? '高于正常值' : '低于正常值');
                                                            _this.$set(item.departItemResults, 'arrow', rett.arrow);
                                                        } else if (!rett.flag) {
                                                            _this.$set(item.departItemResults, 'crisisDegree', "正常");
                                                            _this.$set(item.departItemResults, 'arrow', rett.arrow);
                                                        }
                                                    }
                                                }
            ...
            userInfo().then(res => {
                if (res.success) {
                    if (res.data) {
                        ...
                        this.doctor = res.data.nickname;
                        this.officeId = res.data.departmentId;
                    }
                }
            });
