...
templateJs.joint = function (_this, data) {
    ...
    let checkSignNow = "";
    if (data.groupItems.departResult) {
        if (data.groupItems.departResult.checkSignPath && data.groupItems.departResult.checkSignPath.indexOf("/dcm") > -1) {
            // data.groupItems.departResult.checkSign = data.groupItems.departResult.checkSignPath;
            checkSignNow = data.groupItems.departResult.checkSignPath;
        } else {
            checkSignNow = data.groupItems.departResult.checkSign;
        }
    } else {
        if (data.groupItems.checkSignPath && data.groupItems.checkSignPath.indexOf("/dcm") > -1) {
            // data.groupItems.checkSign = data.groupItems.checkSignPath;
            checkSignNow = data.groupItems.checkSignPath;
        } else {
            checkSignNow = data.groupItems.checkSign;
        }
    }
    ...
    if (isFile != "是") {
        td = {
            name: data.person_name ?? data.personName,
            sex: data.sex,
            age: data.age,
            reportNum: data.test_num ?? data.testNum,
            testNum: data.test_num ?? data.testNum,
            unitName: data.unit_name ?? data.unitName,
            // autograph:data.groupItems.departResult ? data.groupItems.departResult.checkSign : data.groupItems.checkSign,//医生签名
            autograph: checkSignNow,//医生签名
            sectionOffice: data.groupItems.officeName,//科室名称
            createTime: formartDate((data.groupItems.departResult && data.groupItems.departResult.checkDate) || data.groupItems.checkDate || new Date(), "yyyy-MM-dd HH:mm:ss"),//送检时间
            createDate: formartDate((data.groupItems.departResult && data.groupItems.departResult.checkDate) || data.groupItems.checkDate || new Date(), "yyyy-MM-dd"),//送检日期
            inspectTime: formartDate((data.groupItems.departResult && data.groupItems.departResult.checkDate) || data.groupItems.checkDate || new Date(), "yyyy-MM-dd HH:mm:ss"),//检验时间
            inspectDate: formartDate((data.groupItems.departResult && data.groupItems.departResult.checkDate) || data.groupItems.checkDate || new Date(), "yyyy-MM-dd"),//检查日期
        ...
        //使用用登记日期作为报告上的检查日期
        if (_thisJs.$hospitalName && _thisJs.$hospitalName.isUserRegisterTimeReport) {
            td.createTime = formartDate(data.regist_date || new Date(), "yyyy-MM-dd HH:mm:ss");//送检时间
            td.createDate = formartDate(data.regist_date || new Date(), "yyyy-MM-dd");//送检日期
            td.inspectTime = formartDate(data.regist_date || new Date(), "yyyy-MM-dd HH:mm:ss");//检验时间
            td.inspectDate = formartDate(data.regist_date || new Date(), "yyyy-MM-dd");//检查日期
        }
        ...
        if (groupItemName.indexOf("肺功能") > -1) {
            let imgUrlList = data.baseProject[0].departItemResults.imgUrl;
            if (imgUrlList) {
                if (imgUrlList.indexOf(",") > -1) {
                    ...
                    td.imgUrlList.push({img: imgData0[0].replaceAll("/tempFileUrl", "")});
                    let imgData1 = urlSplit.filter(item => item && item.indexOf("-V") > -1);
                    td.imgUrlList.push({img: imgData1[0].replaceAll("/tempFileUrl", "")});
                } else {
                    td.imgUrlList.push({img: imgUrlList.replaceAll("/tempFileUrl", "")});
                }
            } else {
                td.imgUrlList = [];
            }
        ...
        //去除为空的检查项
        if (data && data.baseProject && data.baseProject.length > 0) {
            data.baseProject = data.baseProject.filter(ii => ii.departItemResults && ii.departItemResults.result != undefined && ii.departItemResults.result != null && ii.departItemResults.result.toString().trim().length != 0);
        }
        ...
        /*if(data && data.baseProject){
            let  ybjcData = data.baseProject.filter(ii => ii.departItemResults.officeName && ii.departItemResults.officeName.indexOf("一般检查") > -1);
            if(ybjcData && ybjcData.length > 0){
                let heightData = ybjcData.filter(ii => ii.departItemResults.orderGroupItemProjectName && ii.departItemResults.orderGroupItemProjectName.indexOf("身高") > -1);
                let weightData = ybjcData.filter(ii => ii.departItemResults.orderGroupItemProjectName && ii.departItemResults.orderGroupItemProjectName.indexOf("体重") > -1);
                if(heightData && heightData.length > 0 && heightData[0].result){
                    td.height = heightData[0].result + "cm";//身高
                }
                if(weightData && weightData.length > 0 && weightData[0].result){
                    td.weight = weightData[0].result + "kg";//体重
                }
            }
        }*/
        ...
        if (officeName.indexOf("一般检查") > -1) {
            ...
            if (data.baseProject && data.baseProject.length > 0) {
                td = pureTone(data, td, _this)
                let cyDatasTZ = [];
                let cyDatasTZGD = [];
                if (data.hazardFactorsText ?? data.hazard_factors_text) {
                    let hazardFactorsText = data.hazardFactorsText ?? data.hazard_factors_text;
                    ...
                    if (hazardFactorsText.indexOf("|") > -1 || hazardFactorsText.indexOf("、") > -1) {
                        ...
                        if ((int < hazardFactorsTextArray.length && td.GFA != 0) || (_this.$hospitalName && _this.$hospitalName.isShowHighFrequency)) {
                            ...
                            if (td.leftImgChart["gy"] && td.leftImgChart["gy"].length > 0 && td.rightImgChart["gy"] && td.rightImgChart["gy"].length > 0) {
                                td.leftImgChart = {
                                    ...
                                    "y": [td.leftImgChart["y"][0], td.leftImgChart["y"][1], td.leftImgChart["y"][2], null, null, null],
                                    "gy": [td.leftImgChart["gy"][0], td.leftImgChart["gy"][1], td.leftImgChart["gy"][2], null, null, null]
                                };
                                td.rightImgChart = {
                                    "x": ["500", "1000", "2000", "3000", "4000", "6000"],
                                    "y": [td.rightImgChart["y"][0], td.rightImgChart["y"][1], td.rightImgChart["y"][2], null, null, null],
                                    "gy": [td.rightImgChart["gy"][0], td.rightImgChart["gy"][1], td.rightImgChart["gy"][2], null, null, null]
                                };
                            } else {
                                td.leftImgChart = {
                                    "x": ["500", "1000", "2000", "3000", "4000", "6000"],
            ...
            if (officeName.indexOf("检验科") > -1 && groupItemsName.indexOf("尿常规") > -1) {
                if (data.baseProject && data.baseProject.length > 0) {
                    if (_this.$hospitalName && _this.$hospitalName.urinalysis && _this.$hospitalName.urinalysis.dryChemistry && _this.$hospitalName.urinalysis.dryChemistry) {
                        //筛选干化学项目
                        let dryChemistry = data.baseProject.filter(i => _this.$hospitalName.urinalysis.dryChemistry.indexOf(i.shortName) > -1)
                        //筛选镜检项目
                        let microscope = data.baseProject.filter(i => _this.$hospitalName.urinalysis.microscope.indexOf(i.shortName) > -1)
                        //for循环干化学项目
                        let baseProject = [];
                        ...
                        if (baseProject && baseProject.length) {
                            for (let i = 0; i < baseProject.length; i++) {
                                ...
                                let arrow2 = "";
                                let resultTips2 = "";
                                if (dryChemistry[i] && dryChemistry[i].departItemResults.orderGroupItemProjectName) {
                                    if (dryChemistry[i].departItemResults.orderGroupItemProjectName) {
                                        monitorName = dryChemistry[i].departItemResults.orderGroupItemProjectName;
                                    }
                                    if (dryChemistry[i].departItemResults.result) {
                                        value = dryChemistry[i].departItemResults.result;
                                    }
                                    if (dryChemistry[i].departItemResults.scope) {
                                        reference = dryChemistry[i].departItemResults.scope;
                                    }
                                    if (dryChemistry[i].departItemResults.unitName) {
                                        unit = dryChemistry[i].departItemResults.unitName.indexOf("无") > -1 ? "" : dryChemistry[i].departItemResults.unitName;
                                    }
                                    if (dryChemistry[i].departItemResults.arrow) {
                                        arrow = dryChemistry[i].departItemResults.arrow;
                                    }
                                    if (dryChemistry[i].departItemResults.crisisDegree) {
                                        resultTips = dryChemistry[i].departItemResults.crisisDegree;
                                    }
                                }
                                if (microscope[i] && microscope[i].departItemResults) {
                                    if (microscope[i].departItemResults.orderGroupItemProjectName) {
                                        monitorName2 = microscope[i].departItemResults.orderGroupItemProjectName;
                                    }
                                    if (microscope[i].departItemResults.result) {
                                        value2 = microscope[i].departItemResults.result;
                                    }
                                    if (microscope[i].departItemResults.scope) {
                                        reference2 = microscope[i].departItemResults.scope;
                                    }
                                    if (microscope[i].departItemResults.unitName) {
                                        unit2 = microscope[i].departItemResults.unitName.indexOf("无") > -1 ? "" : microscope[i].departItemResults.unitName;
                                    }
                                    if (microscope[i].departItemResults.arrow) {
                                        arrow2 = microscope[i].departItemResults.arrow;
                                    }
                                    if (microscope[i].departItemResults.crisisDegree) {
                                        resultTips2 = microscope[i].departItemResults.crisisDegree;
                                    }
                                }
            ...
            } else if (officeName.indexOf("检验科") > -1 && groupItemsName.indexOf("外周血") > -1) {
                if (data.baseProject && data.baseProject.length > 0) {
                    let int = 1;
                    data.baseProject.forEach(i => {
                        //单个基础项
            ...
            } else if (officeName.indexOf("检验科") > -1 && groupItemsName.indexOf("血常规") <= -1 && _thisJs.$hospitalName && _thisJs.$hospitalName.isMergeShowJYKSH) {
                if (data.baseProject && data.baseProject.length > 0) {
                    let int = 1;
                    let tableMon = [];
                    data.baseProject.forEach(i => {
                    ...
                    td.tableMonitoring.push({
                        tableMon: tableMon,
                        inspectDoctor: td.inspectDoctor,
                        createTime: td.createTime,
                        inspectTime: td.inspectTime,
        ...
        td.projectNameTemplate = groupItemName;//项目名称
        td.projectTypeName = groupItemName?.includes("B超") ? "B超" : "彩超"
    }
    //肺功能测试曲线展示
    td.showImgUrl = [];
    if (td.imgUrlList && td.imgUrlList.length > 0) {
        td.showImgUrl.push({
            imgUrlList: td.imgUrlList
        })
    }
    return td;
}
    ...
    vitalCapacityShort1.forEach(i => {
        ...
        if (items && items.length > 0) {
            ...
            vitalCapacity1.push({
                ...
                resultTips: (items[0].departItemResults.result && items[0].departItemResults.result.toString().trim() != 0 && items[0].departItemResults.result.toString().trim() != "-" && referenceRange && referenceRange != 0 && referenceRange != "-") ? (((items[0].departItemResults.result.toString().trim() / referenceRange).toFixed(4)) * 100).toFixed(2) : 0,
    ...
    vitalCapacityShort2.forEach(i => {
        ...
        if (items && items.length > 0) {
            ...
            vitalCapacity2.push({
                ...
                value: items[0].departItemResults.result && items[0].departItemResults.result.toString().trim().length > 0 && items[0].departItemResults.result.toString().trim() != "-" ? items[0].departItemResults.result.toString().trim() : 0,
                referenceRange: referenceRange,
                resultTips: (items[0].departItemResults.result && items[0].departItemResults.result.toString().trim() != 0 && items[0].departItemResults.result.toString().trim() != "-" && referenceRange && referenceRange != 0 && referenceRange != "-") ? (((items[0].departItemResults.result.toString().trim() / referenceRange).toFixed(4)) * 100).toFixed(2) : "",
                unit: items[0].departItemResults.unitName.replace(" ", ""),
            });
        } else {
            if (items && items.length > 0) {
...
function gynaecology(data, td) {
    data.baseProject.forEach(i => {
        ...
        if (i.departItemResults.orderGroupItemProjectName) {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "宫体" || i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "子宫体") {
 ...
 *
 * */
function pureTone(data, td, _this) {
    //气体·
    ...
    if (data.baseProject && data.baseProject.length > 0) {
        data.baseProject.forEach(i => {
            ...
            //气体
            if (i.departItemResults.orderGroupItemProjectName) {
                let result = i.departItemResults.deleteId;//未修正
                if (result == null) {
                    if (_this.$hospitalName && _this.$hospitalName.isNotShowGTc6000) {
                        result = 0;
                    } else {
    ...
    //线条区分显示 没有就不显示
    if (gr500 == 0 && gr1000 == 0 && gr2000 == 0 && gr3000 == 0 && gr4000 == 0 && gr6000 == 0) {//骨导右耳
        //骨导
        gr500 = '';
        gr1000 = '';
        gr2000 = '';
...
function ophthalmicRoutine(data, td) {
    data.baseProject.forEach(i => {
        ...
        if (i.departItemResults.orderGroupItemProjectName) {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "裸眼远视力(右)") {
                if (result == 0) {
                    td.nakedEyeRightY = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "裸眼远视力(左)") {
                if (result == 0) {
                    td.nakedEyeLeftY = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "矫正远视力(右)") {
                if (result == 0) {
                    td.correctRightY = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "矫正远视力(左)") {
                if (result == 0) {
                    td.correctLeftY = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "裸眼近视力(右)") {
                if (result == 0) {
                    td.nakedEyeRightJ = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "裸眼近视力(左)") {
                if (result == 0) {
                    td.nakedEyeLeftJ = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "矫正近视力(右)") {
                if (result == 0) {
                    td.correctRightJ = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "矫正近视力(左)") {
                if (result == 0) {
                    td.correctLeftJ = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "裸眼深视力(右)") {
                if (result == 0) {
                    td.nakedEyeRightS = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "裸眼深视力(左)") {
                if (result == 0) {
                    td.nakedEyeLeftS = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "矫正深视力(右)") {
                if (result == 0) {
                    td.correctRightS = "-";
                } else {
            ...
            if (i.departItemResults.orderGroupItemProjectName.replaceAll(" ", "") == "矫正深视力(左)") {
                if (result == 0) {
                    td.correctLeftS = "-";
                } else {
