...
import RectLoading from "@/views/my-components/scmt/rect-loading";
export default {
  components: {
    LangSwitch,
    Footer,
    ...
    getCaptchaImg() {
      ...
      initCaptcha().then((res) => {
        ...
        if (res.success) {
          ...
          this.captchaImg = drawCodeImage + res.data;
        }
      });
    },
    submit() {
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          authorize(this.form).then((res) => {
            if (res.success) {
              // 存储accessToken
              this.setStore("accessToken", res.data.accessToken);
              let redictInfo = res.data;
              // 获取用户信息
              userInfo().then((res) => {
                if (res.success) {
                  // 避免超过大小限制
