    ...
    <div class="search">
            ...
            <Row type="flex" justify="start" :gutter="16">
                <Col :sm="8" :md="8" :lg="8" :xl="6">
                    ...
                    <div class="tree-bar" :style="{ maxHeight: maxHeight }">
                        <Tree ref="tree" :data="data" show-checkbox :render="renderContent"
                ...
                <Col :sm="16" :md="16" :lg="16" :xl="9">
                    <Form ref="form" :model="form" :label-width="110" :rules="formValidate">
                        ...
                        <FormItem label="上级菜单" prop="pid" v-if="form.type == 0 || form.type == 1">
                            <TreeSelect v-model="form.parentId" :data="parentData" clearable/>
                        </FormItem>
                        ...
                        <FormItem label="请求路径" prop="path" v-if="form.type == 1" class="block-tool">
                            <Tooltip placement="right" :max-width="230" transfer
                      ...
                      <FormItem label="显示左侧菜单" prop="showLeft" v-if="form.type == -1">
                        <i-switch size="large" v-model="form.showLeft">
                          <span slot="open">是</span>
                          <span slot="close">否</span>
                        </i-switch>
                      </FormItem>
                        <FormItem label="其他链接" prop="url" v-if="form.type == -1 && !form.isMenu">
                            <Input v-model="form.url" placeholder="Http链接或路由名"/>
                        </FormItem>
                        <FormItem label="打开方式" prop="description" v-if="form.type == -1 && !form.isMenu">
                        ...
                        <FormItem label="前端组件" prop="component" v-if="form.type == 0">
                            <Input v-bind:disabled="form.level == 1 ? true : false" v-model="form.component"/>
                        </FormItem>
                        <FormItem label="第三方链接" prop="url" v-if="form.type == 0 && form.level == 2">
                            <Input v-model="form.url" placeholder="http://" @on-change="changeEditUrl"/>
                            <span class="url-remark">前端组件需为 sys/monitor/monitor 时生效</span>
                        </FormItem>
                        ...
                        <FormItem label="始终显示" prop="showAlways" v-if="form.level == 1" class="block-tool">
                            ...
                            <Tooltip content="当设为不始终显示时，一级菜单下仅有一个子级菜单只会显示该子级菜单，避免用户多次点击" placement="right" transfer
                                     max-width="280" style="display: inline-block !important">
                        ...
                        <FormItem>
                            <Button style="margin-right: 5px" @click="submitEdit" :loading="submitLoading"
                                    type="primary" icon="ios-create-outline">修改并保存
                            </Button>
        ...
        <Modal draggable :title="modalTitle" v-model="menuModalVisible" :mask-closable="false" :width="500"
            ...
            <Form ref="formAdd" :model="formAdd" :label-width="110" :rules="formValidate">
              ...
              <FormItem label="上级菜单" prop="pid" v-if="form.type == 0 || form.type == 1">
                <TreeSelect v-model="formAdd.parentId" :data="parentData" clearable/>
              </FormItem>
              ...
              <FormItem label="显示左侧菜单" prop="showLeft" v-if="formAdd.type == -1">
                <i-switch size="large" v-model="formAdd.showLeft">
                  <span slot="open">是</span>
                  <span slot="close">否</span>
                </i-switch>
              </FormItem>
                <FormItem label="其他链接" prop="url" v-if="formAdd.type == -1 && !formAdd.isMenu">
                    <Input v-model="formAdd.url" placeholder="Http链接或路由名"/>
                </FormItem>
                <FormItem label="打开方式" prop="description" v-if="formAdd.type == -1 && !formAdd.isMenu">
                ...
                <FormItem label="前端组件" prop="component" v-if="formAdd.type == 0">
                    <Input v-model="formAdd.component"  v-bind:disabled="formAdd.level == 1 ? true : false"/>
                </FormItem>
                <FormItem label="第三方链接" prop="url" v-if="formAdd.type == 0 && formAdd.level == 2">
...
<script>
    ...
    import IconChoose from "@/views/my-components/scmt/icon-choose";
            ...
            getAllList() {
                ...
                this.getRequest("/permission/getAllList").then((res) => {
                    ...
                    if (res.success) {
                        ...
                        this.parentData = this.getTreeData(arr);
                    }
                });
            },
            getTreeData(data) {
