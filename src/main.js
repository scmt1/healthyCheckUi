...
import { Tabs, TabPane, InfiniteScroll, Loading } from "element-ui";
Vue.use(Tabs);
Vue.use(TabPane);
...
Vue.use(BaiduMap, {
  ...
  ak: "ZuuPemVedllZhxciVTse47hltFbUC19K",
});

axios.get("/hospitalName.json").then((res) => {
  // 获取public下的json文件数据
  if (res) {
    Vue.prototype.$hospitalName = res;
  }
});
axios.get("/proType.json").then((res) => {
  // 获取public下的json文件数据
  if (res) {
